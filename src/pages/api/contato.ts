...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (nomeRaw.length > 100) return badRequest('O nome deve ter no máximo 100 caracteres.');

    if (!emailRaw) return badRequest('O e-mail é obrigatório.');
    if (!EMAIL_REGEX.test(emailRaw)) return badRequest('Informe um e-mail válido.');
    ...
    if (mensagemRaw.length > 2000) return badRequest('A mensagem deve ter no máximo 2000 caracteres.');
    ...
    try {
      res = await fetch('https://api.brevo.com/v3/smtp/email', {
        ...
        body: JSON.stringify({
          ...
          to: [{ email: 'pedro@cordeirolima.net', name: 'Pedro Cordeiro Lima' }],
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      ...
      return new Response(JSON.stringify({ success: false, error: 'Timeout ao enviar e-mail.' }), {
        status: 504,
        headers: { 'Content-Type': 'application/json' },
      });
    }
